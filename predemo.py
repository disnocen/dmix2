...
def verifysig(txhash,r,s,pub):
    ...
    vk = ecdsa.VerifyingKey.from_string(pub, curve=ecdsa.SECP256k1, hashfunc=sha256) # the default is sha1
...
a_amt_rec = int(a_amt - (fees +fees_third))
b_amt_rec = int(b_amt - (fees +fees_third))
c_amt_rec = int(c_amt - (fees +fees_third))
...
a_txid_vout = btc.getvoutfromamount(a_txid, a_amt)
b_txid_vout = btc.getvoutfromamount(b_txid, b_amt)
c_txid_vout = btc.getvoutfromamount(c_txid, c_amt)
...
a_outs = [{DMix_addr.to_string():round(a_amt-fees,3)}]
a_txid_indmix = btc.sendspecific(a_ins, a_outs, wallet="aliceWallet")
a_txid_indmix_vout = btc.getvoutfromamount(a_txid_indmix, round(a_amt-fees,3), wallet="aliceWallet")
...
b_outs = [{DMix_addr.to_string():round(b_amt-fees,3)}]
b_txid_indmix = btc.sendspecific(b_ins, b_outs, wallet="bobWallet")
b_txid_indmix_vout = btc.getvoutfromamount(b_txid_indmix, round(b_amt-fees,3), wallet="bobWallet")
...
c_outs = [{DMix_addr.to_string():round(c_amt-fees,3)}]
c_txid_indmix = btc.sendspecific(c_ins, c_outs, wallet="carolWallet")
c_txid_indmix_vout = btc.getvoutfromamount(c_txid_indmix, round(c_amt-fees,3), wallet="carolWallet")
print("Carol's transaction to DMix:", c_txid_indmix)

indmix_tx_data = [{"txid":a_txid_indmix,"vout":a_txid_indmix_vout,"script":btc.getscriptfromvout(a_txid_indmix, a_txid_indmix_vout,wallet="aliceWallet"),"pub":a_pub}, {"txid":b_txid_indmix,"vout":b_txid_indmix_vout,"script":btc.getscriptfromvout(b_txid_indmix, b_txid_indmix_vout,wallet="bobWallet"),"pub":b_pub}, {"txid":c_txid_indmix,"vout":c_txid_indmix_vout,"script":btc.getscriptfromvout(c_txid_indmix, c_txid_indmix_vout,wallet="carolWallet"), "pub":c_pub}]
...
print()
print("DMix balance:", abs(btc.gettransaction(a_txid_indmix,wallet="aliceWallet")["amount"])+abs(btc.gettransaction(b_txid_indmix,wallet="bobWallet")["amount"])+abs(btc.gettransaction(c_txid_indmix,wallet="carolWallet")["amount"]))
input("Press Enter to continue...")
...
dmix_tx_in0 = TxInput(indmix_tx_data[0]["txid"], indmix_tx_data[0]["vout"],Script(indmix_tx_data[0]["script"].split()))
dmix_tx_in1 = TxInput(indmix_tx_data[1]["txid"], indmix_tx_data[1]["vout"],Script(indmix_tx_data[1]["script"].split()))
dmix_tx_in2 = TxInput(indmix_tx_data[2]["txid"], indmix_tx_data[2]["vout"],Script(indmix_tx_data[2]["script"].split()))

addr_out0 = P2pkhAddress(all_out_addresses[0])
dmix_tx_out0 = TxOutput(to_satoshis(amount_out), Script(['OP_DUP', 'OP_HASH160', addr_out0.to_hash160(),'OP_EQUALVERIFY', 'OP_CHECKSIG']))

addr_out1 = P2pkhAddress(all_out_addresses[1])
dmix_tx_out1 = TxOutput(to_satoshis(amount_out), Script(['OP_DUP', 'OP_HASH160', addr_out1.to_hash160(),'OP_EQUALVERIFY', 'OP_CHECKSIG']))

addr_out2 = P2pkhAddress(all_out_addresses[2])
dmix_tx_out2 = TxOutput(to_satoshis(amount_out), Script(['OP_DUP', 'OP_HASH160', addr_out2.to_hash160(),'OP_EQUALVERIFY', 'OP_CHECKSIG']))

addr_out3 = P2pkhAddress(all_out_addresses[3])
dmix_tx_out3 = TxOutput(to_satoshis(amount_out), Script(['OP_DUP', 'OP_HASH160', addr_out3.to_hash160(),'OP_EQUALVERIFY', 'OP_CHECKSIG']))

addr_out4 = P2pkhAddress(all_out_addresses[4])
dmix_tx_out4 = TxOutput(to_satoshis(amount_out), Script(['OP_DUP', 'OP_HASH160', addr_out4.to_hash160(),'OP_EQUALVERIFY', 'OP_CHECKSIG']))

addr_out5 = P2pkhAddress(all_out_addresses[5])
dmix_tx_out5 = TxOutput(to_satoshis(amount_out), Script(['OP_DUP', 'OP_HASH160', addr_out5.to_hash160(),'OP_EQUALVERIFY', 'OP_CHECKSIG']))

addr_out6 = P2pkhAddress(all_out_addresses[6])
dmix_tx_out6 = TxOutput(to_satoshis(amount_out), Script(['OP_DUP', 'OP_HASH160', addr_out6.to_hash160(),'OP_EQUALVERIFY', 'OP_CHECKSIG']))

# dmix_tx = Transaction([dmix_tx_in0], [dmix_tx_out0,dmix_tx_out1,dmix_tx_out2,dmix_tx_out3])
dmix_tx = Transaction([dmix_tx_in0,dmix_tx_in1,dmix_tx_in2], [dmix_tx_out0,dmix_tx_out1,dmix_tx_out2,dmix_tx_out3, dmix_tx_out4, dmix_tx_out5, dmix_tx_out6])

digest0=dmix_tx.get_transaction_digest(0,Script(indmix_tx_data[0]["script"].split()),sighash=SIGHASH_ALL|SIGHASH_ANYONECANPAY)
digest1=dmix_tx.get_transaction_digest(1,Script(indmix_tx_data[1]["script"].split()),sighash=SIGHASH_ALL|SIGHASH_ANYONECANPAY)
digest2=dmix_tx.get_transaction_digest(2,Script(indmix_tx_data[2]["script"].split()),sighash=SIGHASH_ALL|SIGHASH_ANYONECANPAY)
...
subprocess.check_output(["/bin/sh", "-c","./demo/thsign.sh "+digest0.hex() ]).decode('utf-8').strip()
...
subprocess.check_output(["/bin/sh", "-c","./demo/thsign.sh "+digest2.hex() ]).decode('utf-8').strip()
with open('./signature.json', 'r') as ff:
    sig = eval(ff.read())
ff.close()
r= sig["r"]
